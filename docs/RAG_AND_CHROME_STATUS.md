...
#### На macOS (Рекомендовано)

```bash
# Через Homebrew (найпростіше)
...
| Установка | `brew install google-chrome` |

### Redis Status

| Метрика | Статус |
|---------|--------|
| Redis Server | ❌ Не встановлено |
| Потрібно? | ✅ ОБОВ'ЯЗКОВО |
| Установка | `brew install redis` |
| Запуск | `brew services start redis` |
...
- ✅ Потрібен для Vision features
- ✅ Потрібен для AppleScript execution
- ✅ Встановлення: `brew install google-chrome`
- ✅ Або через setup: `./setup.sh`

### ✅ Redis

**ОБОВ'ЯЗКОВО ПОТРІБЕН!**

- ✅ Потрібен для state management
- ✅ Потрібен для checkpoint persistence
- ✅ Встановлення: `brew install redis`
- ✅ Запуск: `brew services start redis`
- ✅ Або через setup: `./setup.sh`
...
# 2. Запустити setup (встановить все)
chmod +x setup.sh
./setup.sh

# 3. Редагувати .env з вашими API ключами
vim .env
...
**Status:** ✅ **RAG OPERATIONAL** | ✅ **CHROME REQUIRED** | ✅ **REDIS REQUIRED**  
**Date:** 11 грудня 2025, 00:35 UTC+02:00
